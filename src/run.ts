...
log("[RADAR]", colors.bright, `Starting in ${colors.magenta}${mode.toUpperCase()}${colors.reset} mode`);
log("[RADAR]", colors.magenta, "━".repeat(60));
console.log("\n");

// Start Python backend server (using venv)
const backend: ChildProcess = spawn("./venv/bin/python", ["src/radar_tracks_server.py"], {
	cwd: process.cwd(),
	stdio: "pipe",
...
log("[RADAR]", colors.bright, `Press ${colors.red}Ctrl+C${colors.reset} to stop both servers`);
console.log("\n");
